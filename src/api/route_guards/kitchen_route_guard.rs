...
pub struct KitchenGuard;

#[rocket::async_trait]
impl<'r> FromRequest<'r> for KitchenGuard {
      ...
      match req.headers().get_one("UserGroup") {
          Some(user_group_string) => {
              let user_group_validator = req
                  ...
                  .unwrap();
              let valid = user_group_validator.validate_user_group(user_group_string.to_string(), UserGroup::Admin) || user_group_validator.validate_user_group(user_group_string.to_string(), UserGroup::Kitchen);
              match valid {
                  true => Outcome::Success(KitchenGuard),
                  false => {
...
impl<'a> OpenApiFromRequest<'a> for KitchenGuard {
  fn from_request_input(
      ...
      let security_scheme = SecurityScheme {
          description: Some("Usuário Cozinha ou Admin para accessar".to_owned()),
